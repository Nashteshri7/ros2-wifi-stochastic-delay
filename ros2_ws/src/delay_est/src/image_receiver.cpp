...
class Receiver : public rclcpp::Node {
    public:
        Receiver(uint32_t width, uint32_t height) : Node("receiver"), width_(width), height_(height) {
            publisher_ = this->create_publisher<Image>("/delay_est/q_m", QUEUE_DEPTH_);
            subscriber_ = this->create_subscription<Image>("/delay_est/q_d", QUEUE_DEPTH_, std::bind(&Receiver::subscriptionCallback, this, _1));
    ...
    private:
        ...
        void dataCollectionCallback() {
            ...
            if(log_file_.is_open()) {
                ...
                std::string file_name = std::to_string(width_) + "x" + std::to_string(height_) + "/" + "sender_to_receiver.csv";
...
int main(int argc, char* argv[]) {
    ...
    for(size_t r = 0; r < 4; r++) {
        ...
        auto receiver_node = std::make_shared<Receiver>(w, h);
